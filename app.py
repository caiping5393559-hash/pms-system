...
def service_account_info():
    ...
    if raw_b64:
        ...
        return SERVICE_ACCOUNT_CACHE
    for path in [Path(os.environ.get("GOOGLE_APPLICATION_CREDENTIALS", "")), BASE / "firebase-key.json", BASE / "firebase-adminsdk.json"]:
    ...
    for line in unfolded:
        ...
        if line == "END:VEVENT":
            ...
            if in_event and checkin:
                ...
                if checkout > checkin:
                    events.append({"room_id": room_id, "platform": platform, "guest": "", "checkin": checkin, "checkout": checkout, "status": current.get("summary") or f"{platform} iCal", "source": "ical"})
            in_event = False
            continue
