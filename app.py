...
PMS_APP_VERSION = "2026-07-08-v91-settlement-default-done-fix"
...
if "icalFeedCache" not in STATE_KEYS:
    STATE_KEYS.append("icalFeedCache")
...
def _pms_channel_feed_stamp(value=""):
    text = str(value or "").strip()
    if text:
        try:
...
def _pms_channel_build_feed_from_state(state, feed_id, generated_at=""):
    normalized_feed_id = str(feed_id or "").replace(".ics", "")
    room_id, target_channel_id, target_listing = _pms_channel_feed_target(state, normalized_feed_id)
    if normalized_feed_id in _pms_emergency_empty_feed_ids() and not target_channel_id:
        return _pms_empty_ical_calendar("PMS Anti Overbooking"), {"event_count": 0, "room_id": room_id, "target_channel_id": target_channel_id}
    if not room_id:
        return None, {}

    dtstamp = _pms_channel_feed_stamp(generated_at)
    event_rows = []
    seen = set()
    for booking in state.get("bookings", []):
        ...
        if "@pms-system" in marker_text or "generated by pms anti-overbooking feed" in marker_text:
            continue
        checkin = str(booking.get("checkin") or "")
        checkout = str(booking.get("checkout") or "")
        if not checkin or not checkout or checkout <= checkin:
            continue
        ...
        if dedupe in seen:
            continue
        seen.add(dedupe)
        uid_seed = "|".join([str(normalized_feed_id), str(booking.get("channel_listing_id") or ""), str(booking.get("external_event_uid") or booking.get("id") or ""), checkin, checkout, kind])
        uid = hashlib.sha1(uid_seed.encode("utf-8")).hexdigest() + "@pms-system"
        event_rows.append((checkin, checkout, uid))

    event_lines = []
    for checkin, checkout, uid in sorted(event_rows, key=lambda row: (row[0], row[1], row[2])):
        event_lines.append(
            "BEGIN:VEVENT\r\n"
            f"UID:{uid}\r\n"
            f"DTSTAMP:{dtstamp}\r\n"
    ...
    if target_listing:
        note = str(target_listing.get("channel_note") or "").strip()
        title += " - " + (note or target_listing.get("platform") or "channel")
    return (
        "BEGIN:VCALENDAR\r\n"
        "VERSION:2.0\r\n"
        "PRODID:-//PMS System//Channel Feed//EN\r\n"
        ...
        f"X-WR-CALNAME:{_pms_channel_ics_escape(title)}\r\n"
        + "\r\n".join(event_lines)
        + "\r\nEND:VCALENDAR\r\n"
    ), {"event_count": len(event_rows), "room_id": room_id, "target_channel_id": target_channel_id}
...
def _pms_channel_make_feed(feed_id):
    normalized_feed_id = str(feed_id or "").replace(".ics", "")
    state = normalize_state(load_main_state())
    for item in state.get("icalFeedCache", []):
        if isinstance(item, dict) and item.get("feed_id") == normalized_feed_id and item.get("ics"):
            return item.get("ics")
    feed, _ = _pms_channel_build_feed_from_state(state, normalized_feed_id)
    return feed
