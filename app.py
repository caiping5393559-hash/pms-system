...
PMS_PATCH_VERSION = "2026-07-01-calendar-cleaning-dedupe-v7"
...
final_ui_override = r'''
(function(){
  const VERSION='2026-07-01-calendar-cleaning-dedupe-v7';
  ...
  function applyState(data){
    ...
    if(Array.isArray(state.icalEventArchive))setList('icalEventArchive',state.icalEventArchive);
  ...
  async function syncPropertyIcal(propertyId,btn){
    ...
    S.syncResults[id]={kind:'loading',text:'同步中：正在直接读取平台 iCal...'};
    if(typeof window.renderRoomSettings==='function')window.renderRoomSettings();
    try{
      ...
      const msg=e&&e.name==='AbortError'?'同步超过 90 秒已停止，请先点检查确认 iCal 链接是否有效。':(e&&e.message?e.message:String(e||'未知错误'));
