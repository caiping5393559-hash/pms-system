(function(){
  ...
  function renderSixMonthStatsImpl(){
    ...
    const stats = rooms.map(room => {
      ...
      const available = Math.max(0, bookable - orderDays.size);
      ...
      return {
        ...
        cleaningFee,
        html: `<tr>${showProp?`<td>${esc(propName(roomPropId(room.id)))}</td>`:''}<td>${esc(roomName(room.id))}</td><td>${real.length}</td><td>${orderDays.size}</td><td>${lockDays.size}</td><td>${available}</td><td>${rate}</td><td>${money(cleaningFee)}</td></tr>`
      };
    });
    ...
    const totals = stats.reduce((sum, item) => ({
      ...
      availableNights: sum.availableNights + item.availableNights,
      cleaningFees: sum.cleaningFees + item.cleaningFee
    }), {orders:0, orderNights:0, lockNights:0, availableNights:0, cleaningFees:0});
    const totalBookableNights = totals.orderNights + totals.availableNights;
    ...
    const summary = rows ? `<tr class="stats-summary-row">${showProp?`<td>${esc(t('owner.table.summary'))}</td>`:''}<td>${esc(t('owner.table.roomsCount', {count:stats.length}))}</td><td>${totals.orders}</td><td>${totals.orderNights}</td><td>${totals.lockNights}</td><td>${totals.availableNights}</td><td>${overallRate}</td><td>${esc(t('owner.table.averagePrefix'))} ${money(averageCleaningFee)}</td></tr>` : '';
    el.innerHTML = `<table><tr>${showProp?`<th>${esc(t('owner.table.property'))}</th>`:''}<th>${esc(t('owner.table.room'))}</th><th>${esc(t('owner.table.orders'))}</th><th>${esc(t('owner.table.orderNights'))}</th><th>${esc(t('owner.table.lockNights'))}</th><th>${esc(t('owner.table.availableNights'))}</th><th>${esc(t('owner.table.occupancy'))}</th><th>${esc(t('owner.table.cleaningFee'))}</th></tr>${rows || `<tr><td colspan="${showProp?8:7}">${esc(t('owner.table.noRooms'))}</td></tr>`}${summary}</table>`;
  }
