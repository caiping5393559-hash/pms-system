(function(){
  ...
  const I18N = {
    'zh-CN': {
      ...
      'owner.table.property':'房源','owner.table.room':'房间','owner.table.orders':'订单数','owner.table.orderNights':'订单晚数','owner.table.lockNights':'不开放锁定晚数','owner.table.pastUnbookedNights':'过去未订晚数','owner.table.futureAvailableNights':'未来可订晚数','owner.table.occupancy':'预订率','owner.table.cleaningFee':'单次保洁费','owner.table.summary':'汇总','owner.table.roomsCount':'{count}个房间','owner.table.averagePrefix':'平均','owner.table.noRooms':'当前筛选没有房间','owner.table.checkin':'入住/开始','owner.table.checkout':'退房/结束','owner.table.type':'类型','owner.table.booking':'预订','owner.table.locked':'锁定','owner.table.source':'来源','owner.table.guest':'客人','owner.table.status':'状态','owner.table.dateNotes':'日期事项','owner.table.noBookings':'当前日期范围没有预订'
    },
    'en-US': {
      ...
      'owner.table.property':'Property','owner.table.room':'Room','owner.table.orders':'Orders','owner.table.orderNights':'Booked nights','owner.table.lockNights':'Blocked nights','owner.table.pastUnbookedNights':'Past unbooked nights','owner.table.futureAvailableNights':'Future available nights','owner.table.occupancy':'Occupancy','owner.table.cleaningFee':'Cleaning fee','owner.table.summary':'Summary','owner.table.roomsCount':'{count} rooms','owner.table.averagePrefix':'Avg.','owner.table.noRooms':'No rooms match the current filter','owner.table.checkin':'Check-in / start','owner.table.checkout':'Check-out / end','owner.table.type':'Type','owner.table.booking':'Booking','owner.table.locked':'Blocked','owner.table.source':'Source','owner.table.guest':'Guest','owner.table.status':'Status','owner.table.dateNotes':'Date notes','owner.table.noBookings':'No reservations in this date range'
    },
    'es-ES': {
      ...
      'owner.table.property':'Propiedad','owner.table.room':'Habitación','owner.table.orders':'Pedidos','owner.table.orderNights':'Noches reservadas','owner.table.lockNights':'Noches bloqueadas','owner.table.pastUnbookedNights':'Noches pasadas sin reserva','owner.table.futureAvailableNights':'Noches futuras disponibles','owner.table.occupancy':'Ocupación','owner.table.cleaningFee':'Tarifa limpieza','owner.table.summary':'Resumen','owner.table.roomsCount':'{count} habitaciones','owner.table.averagePrefix':'Prom.','owner.table.noRooms':'No hay habitaciones con este filtro','owner.table.checkin':'Entrada / inicio','owner.table.checkout':'Salida / fin','owner.table.type':'Tipo','owner.table.booking':'Reserva','owner.table.locked':'Bloqueo','owner.table.source':'Fuente','owner.table.guest':'Huésped','owner.table.status':'Estado','owner.table.dateNotes':'Notas de fecha','owner.table.noBookings':'No hay reservas en este rango'
    }
  };
  ...
  function ensureStyles(){
    ...
    style.textContent = `
      ...
      .future-stats-scroll table{width:max-content;min-width:860px;margin:0}
      ...
      @media(max-width:600px){
        ...
        .future-stats-scroll table{min-width:820px;font-size:13px}
  ...
  function renderSixMonthStatsImpl(){
    ...
    const selectedDays = calendarDaysForRange(range);
    ...
    const stats = rooms.map(room => {
      ...
      locks.forEach(b => dateRange(b.checkin, addDay(b.checkout,-1)).forEach(d => { if(d >= range.start && d < range.endExclusive && !orderDays.has(d)) lockDays.add(d); }));
      const bookable = Math.max(0, range.dayCount - lockDays.size);
      const isOpenNight = day => !orderDays.has(day) && !lockDays.has(day);
      const pastUnbookedNights = selectedDays.filter(day => day < currentDate && isOpenNight(day)).length;
      const futureAvailableNights = selectedDays.filter(day => day >= currentDate && isOpenNight(day)).length;
      const rate = bookable ? Math.round(orderDays.size / bookable * 1000) / 10 + '%' : '-';
      const cleaningFee = Number(targetFee(room.id,'room') || 0);
      return {
        orders: real.length,
        orderNights: orderDays.size,
        lockNights: lockDays.size,
        pastUnbookedNights,
        futureAvailableNights,
        cleaningFee,
        html: `<tr>${showProp?`<td>${esc(propName(roomPropId(room.id)))}</td>`:''}<td>${esc(roomName(room.id))}</td><td>${real.length}</td><td>${orderDays.size}</td><td>${lockDays.size}</td><td>${pastUnbookedNights}</td><td>${futureAvailableNights}</td><td>${rate}</td><td>${money(cleaningFee)}</td></tr>`
      };
    });
    const rows = stats.map(item => item.html).join('');
    const totals = stats.reduce((sum, item) => ({
      orders: sum.orders + item.orders,
      orderNights: sum.orderNights + item.orderNights,
      lockNights: sum.lockNights + item.lockNights,
      pastUnbookedNights: sum.pastUnbookedNights + item.pastUnbookedNights,
      futureAvailableNights: sum.futureAvailableNights + item.futureAvailableNights,
      cleaningFees: sum.cleaningFees + item.cleaningFee
    }), {orders:0, orderNights:0, lockNights:0, pastUnbookedNights:0, futureAvailableNights:0, cleaningFees:0});
    const totalBookableNights = totals.orderNights + totals.pastUnbookedNights + totals.futureAvailableNights;
    const overallRate = totalBookableNights ? (Math.round(totals.orderNights / totalBookableNights * 1000) / 10 + '%') : '-';
    const averageCleaningFee = stats.length ? totals.cleaningFees / stats.length : 0;
    const summary = rows ? `<tr class="stats-summary-row">${showProp?`<td>${esc(t('owner.table.summary'))}</td>`:''}<td>${esc(t('owner.table.roomsCount', {count:stats.length}))}</td><td>${totals.orders}</td><td>${totals.orderNights}</td><td>${totals.lockNights}</td><td>${totals.pastUnbookedNights}</td><td>${totals.futureAvailableNights}</td><td>${overallRate}</td><td>${esc(t('owner.table.averagePrefix'))} ${money(averageCleaningFee)}</td></tr>` : '';
    el.innerHTML = `<table><tr>${showProp?`<th>${esc(t('owner.table.property'))}</th>`:''}<th>${esc(t('owner.table.room'))}</th><th>${esc(t('owner.table.orders'))}</th><th>${esc(t('owner.table.orderNights'))}</th><th>${esc(t('owner.table.lockNights'))}</th><th>${esc(t('owner.table.pastUnbookedNights'))}</th><th>${esc(t('owner.table.futureAvailableNights'))}</th><th>${esc(t('owner.table.occupancy'))}</th><th>${esc(t('owner.table.cleaningFee'))}</th></tr>${rows || `<tr><td colspan="${showProp?9:8}">${esc(t('owner.table.noRooms'))}</td></tr>`}${summary}</table>`;
  }
  function renderOwnerBookingsImpl(){
    ...
    rows.sort((a,b) => String(a.checkin).localeCompare(String(b.checkin)) || roomName(a.room_id).localeCompare(roomName(b.room_id),'zh-Hans-CN') || (Number(isLockedBooking(a)) - Number(isLockedBooking(b))) || String(a.checkout).localeCompare(String(b.checkout)));
    const showProp = ownerPropIds().length !== 1;
    const el = qs('ownerBookings');
    if(!el) return;
    el.innerHTML = `<table><tr><th>${esc(t('owner.table.checkin'))}</th><th>${esc(t('owner.table.checkout'))}</th>${showProp?`<th>${esc(t('owner.table.property'))}</th>`:''}<th>${esc(t('owner.table.room'))}</th><th>${esc(t('owner.table.type'))}</th><th>${esc(t('owner.table.source'))}</th><th>${esc(t('owner.table.guest'))}</th><th>${esc(t('owner.table.status'))}</th><th>${esc(t('owner.table.dateNotes'))}</th></tr>${rows.length ? rows.map(b => {
      const locked = isLockedBooking(b);
      return `<tr class="${locked?'lock-row':''}"><td>${esc(b.checkin)}</td><td>${esc(b.checkout)}</td>${showProp?`<td>${esc(propName(roomPropId(b.room_id)))}</td>`:''}<td><span class="badge">${esc(roomName(b.room_id))}</span></td><td>${bookingTypeBadge(b)}</td><td>${bookingSourceBadges(b)}</td><td>${locked?'':esc(b.guest || '')}</td><td>${locked?`${esc(lockReason(b))}`:esc(displayDataText(b.status || ''))}</td><td>${getRoomNotes().filter(n => n.room_id === b.room_id && n.date >= b.checkin && n.date <= b.checkout).length}</td></tr>`;
    }).join('') : `<tr><td colspan="${showProp?9:8}">${esc(t('owner.table.noBookings'))}</td></tr>`}</table>`;
  }
