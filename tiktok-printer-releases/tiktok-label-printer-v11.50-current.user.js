// ==UserScript==
// @name         WINDOWS电脑 TikTok标签 v11.50 清理稳定版
// @name:zh-CN   WINDOWS电脑 TikTok标签 v11.50 清理稳定版
// @namespace    local-tiktok-label-printer-windows
// @version      11.50
// @description  v11.50：快捷打印当日订单改用快速扫描/快速翻页，减少 100+ 单等待时间；批量重复打印仍硬拦截。
// @description:zh-CN v11.50：快捷打印当日订单改用快速扫描/快速翻页，减少 100+ 单等待时间；批量重复打印仍硬拦截。
...
async function quickPrintTodayAuctionAndGiveaway(){
    ...
    try{
        ...
        while(page<=80){
            ...
            const pageInfos=blocks.map(infoFromBlock).filter(x=>x&&x.order);
