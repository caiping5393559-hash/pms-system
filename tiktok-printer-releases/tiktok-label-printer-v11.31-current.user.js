// ==UserScript==
// @name         WINDOWS电脑 TikTok标签 v11.31 清理稳定版
// @name:zh-CN   WINDOWS电脑 TikTok标签 v11.31 清理稳定版
// @namespace    local-tiktok-label-printer-windows
// @version      11.31
// @description  v11.31：新增中英文主菜单、版本下载中心、监听暂停原因提示
// @description:zh-CN v11.31：新增中英文主菜单、版本下载中心、监听暂停原因提示
// @match        https://*/*
...
const RELEASE_COLLECTION="tiktok_script_releases";
const RELEASE_DOC_LATEST="latest";
...
function normalizeReleaseDoc(d){
    d=d||{};
    return {
        version:String(d.version||d.__docId||"").replace(/^v/i,""),
        ...
        filename:d.filename||("tiktok-label-printer-v"+String(d.version||d.__docId||SCRIPT_VERSION).replace(/^v/i,"")+"-current.user.js"),
...
async function loadReleaseHistory(){
    try{
        const docs=await listRestCollection(RELEASE_COLLECTION,120);
        const rows=docs
            .map(normalizeReleaseDoc)
            .filter(x=>x.version&&x.version!==RELEASE_DOC_LATEST)
...
async function loadReleaseDoc(version){
    const id=version&&version!=="latest"?String(version).replace(/^v/i,""):RELEASE_DOC_LATEST;
...
async function saveReviewOrdersCloudRest(infos,scanId,page,meta,nowLocal,nowIso,nowMs){
    let writes=[];
    let count=0;
...
async function saveReviewOrdersCloud(infos,scanId,page){
    try{
        ...
        if(shouldUseCloudRestFirst()||!await waitForFirebaseCloud(1500)){
            ...
            tkCloudLastAt=now();
            tkCloudStatus="云同步：复核已保存 "+restResult.count+" 单（REST备用）";
            updateStablePanelSoon();
            return restResult;
        }
        let count=0;
