// ==UserScript==
// @name         WINDOWS电脑 TikTok标签 v11.52 清理稳定版
// @name:zh-CN   WINDOWS电脑 TikTok标签 v11.52 清理稳定版
// @namespace    local-tiktok-label-printer-windows
// @version      11.52
// @description  v11.52：Free sample 识别为样品订单，并在复核统计/商品分析中从橱窗订单里单独拆出。
// @description:zh-CN v11.52：Free sample 识别为样品订单，并在复核统计/商品分析中从橱窗订单里单独拆出。
// @match        https://seller-us.tiktok.com/*
// @match        https://seller.tiktokglobalshop.com/*
// @match        https://*.tiktokglobalshop.com/*
...
const GIVEAWAY_NOTE="Giveaway";

const SCRIPT_VERSION="11.52";
...
const RELEASE_HISTORY_FALLBACK=[
    {version:"11.52",date:"2026-06-29",notesZh:"Free sample 订单改为识别成样品订单；复核统计、商品维度分析和订单明细里从橱窗订单单独拆出样品订单，避免样品被算进橱窗订单。",notesEn:"Free sample orders are now classified as sample orders. Review statistics, product analysis, and order details separate sample orders from showcase orders so samples are no longer counted as showcase orders."},
    {version:"11.51",date:"2026-06-29",notesZh:"快捷打印和复核改为安全采集：降低滚动速度、缩小步长并自动多次补扫；当 100/Page 且还有下一页但采集数量不足时，停止打印/复核并提示漏扫风险，避免少单被当成完整结果。",notesEn:"Quick print and review now use safer collection: slower smaller scroll steps with automatic rescans; when 100/Page still has a next page but collected orders are fewer than expected, printing/review stops with a missing-row warning instead of treating the partial page as complete."},
    {version:"11.50",date:"2026-06-29",notesZh:"快捷打印当日订单性能优化：只在快捷扫描中启用快速采集和快速翻页，减少 100+ 单等待时间；复核扫描仍保留稳模式；状态栏显示每页和总扫描耗时。",notesEn:"Quick-print performance optimization: today's quick print now uses fast page collection and fast pagination to reduce waits for 100+ orders; review scanning keeps the safer steady mode; status now shows per-page and total scan time."},
    {version:"11.49",date:"2026-06-28",notesZh:"批量打印安全升级：当前页批量、快捷打印、复核批量只要发现本地或云端已打印订单就停止整批；单独补打保留，但会先确认订单号、Title、Note、本地/云端打印次数。",notesEn:"Batch print safety upgrade: current-page batch, quick print, and review batch stop when any selected order was already printed locally or in the cloud; single-order reprint is still supported with an explicit confirmation showing order ID, title, note, and print counts."},
    {version:"11.48",date:"2026-06-28",notesZh:"新增打印前安全闸：同批重复订单号直接拦截；Title 为空、未识别或疑似字段错位直接拦截；同一订单历史 Title 与当前 Title 冲突直接拦截；当前页/快捷/复核批量打印会先查云端打印次数，云端已打印时需要二次确认。",notesEn:"Added print safety gates: duplicate order IDs in one batch are blocked; empty/unrecognized/suspicious titles are blocked; title conflicts with local print history are blocked; current-page, quick, and review batch printing now checks cloud print counts and asks for confirmation when an order was already printed."},
    {version:"11.47",date:"2026-06-28",notesZh:"收窄标签订单号后 6 位数字，并增加右侧安全边距，避免最后一位打印出格。",notesEn:"Narrowed the final six order digits and added right-side safety spacing to prevent the last digit from printing outside the label."},
    {version:"11.46",date:"2026-06-27",notesZh:"调整 50x30 标签右侧三段文字布局：订单号、Title、日期时间之间增加段距，避免三段挤在一起。",notesEn:"Adjusted the 50x30 label text layout: increased spacing between order ID, title, and date/time on the right side."},
    {version:"11.45",date:"2026-06-27",notesZh:"修复本地 Chrome 扩展报错：打印窗口 doc.write 中的内联脚本违反 CSP。现在打印窗口不再包含 script 标签，发送打印任务/关闭按钮由外层插件绑定。",notesEn:"Fixed local Chrome extension CSP errors caused by inline scripts inside the printed iframe. The print iframe now contains no script tag; send/close buttons are bound by the outer plugin code."},
    {version:"11.44",date:"2026-06-27",notesZh:"所有补打、当前页批量、复核打印、快捷打印统一改为可见确认窗口发送打印任务，避免隐藏 iframe 的自动 window.print 被 Chrome 或扩展环境拦截。",notesEn:"All reprint, current-page batch, review print, and quick print flows now use a visible confirmation window to send print jobs, avoiding hidden iframe window.print being blocked by Chrome or extension mode."},
    {version:"11.43",date:"2026-06-27",notesZh:"修复快捷打印今日拍卖扫描完成后打印机没有收到任务的问题：扫描完成后弹出确认按钮，由最后一次用户点击触发真正打印，避免 Chrome 拦截异步 window.print。",notesEn:"Fixed quick auction print not reaching the printer after scanning: after scan completion, a confirmation button now triggers the actual print from a user click, avoiding Chrome blocking asynchronous window.print."},
    {version:"11.42",date:"2026-06-27",notesZh:"新增快捷打印今日 Auction/Giveaway：自动翻页扫描当天订单，筛选非Canceled、打印次数0、有Note的 LIVE Auction；Giveaway 也会打印，Note 写为 Giveaway 加原备注，最后按 Title 排序一次性打印。",notesEn:"Added quick print for today's Auction/Giveaway orders: auto-paginates today's orders, selects non-canceled, zero-print, noted LIVE Auction orders; Giveaway also prints with Note as Giveaway plus original seller note, then prints once sorted by Title."},
    {version:"11.41",date:"2026-06-27",notesZh:"串改猴匹配范围从所有 HTTPS 页面收窄到 TikTok Seller 页面；云同步改为按需启动，减少只用当前页打印时的启动卡顿。",notesEn:"Reduced the Tampermonkey match scope from all HTTPS pages to TikTok Seller pages only; cloud sync now starts on demand to reduce startup lag when using current-page printing."},
    {version:"11.40",date:"2026-06-27",notesZh:"应用新版 50x30 标签布局：二维码 19mm，右侧订单号、标题、日期放大并整体下移压紧。",notesEn:"Applied the new 50x30 label layout: 19mm QR code and larger, lower, tighter right-side order ID, title, and date."},
    {version:"11.39",date:"2026-06-26",notesZh:"放大 50x30 标签二维码到 18mm，并调整右侧订单号、标题、时间位置，避免压住二维码。",notesEn:"Enlarged the 50x30 label QR code to 18mm and shifted the order ID, title, and time to avoid overlap."},
...
const UI_TEXT={
    ...
    en:{
        ...
        quickTodayAuction:"Quick Print Today Auction",
...
const UI_DYNAMIC_TRANSLATIONS=[
    ...
    ["非Canceled 样品订单","Non-Canceled Sample Orders"],
    ["样品订单 GMV","Sample Orders GMV"],
...
let quickTodayAuctionRunning=false;
...
function giveawayPrintNoteFromBase(note){
    const base=stripPrintedMark(note);
    if(!base)return GIVEAWAY_NOTE;
    if(/^giveaway\b/i.test(base))return base;
    return GIVEAWAY_NOTE+" "+base;
}

function effectiveNote(info){
    const base=stripPrintedMark((info&&info.cleanNote)||(info&&info.note)||"");
    if(isGiftOrder(info))return giveawayPrintNoteFromBase(base);
    return base;
}
...
async function scrollMainToBottom(opt){
    opt=opt||{};
    ...
    await wait(Number(opt.waitMs||(opt.fast?250:800)));
}

async function collectCurrentListOrderBlocksByScroll(opt){
    opt=opt||{};
    const fast=!!opt.fast;
    const topWaitMs=Number(opt.topWaitMs||(fast?180:650));
    const stepWaitMs=Number(opt.stepWaitMs||(fast?180:650));
    const finalWaitMs=Number(opt.finalWaitMs||(fast?80:300));
    const restoreWaitMs=Number(opt.restoreWaitMs||(fast?0:300));
    const maxLoops=Number(opt.maxLoops||(fast?10:22));
    const stableNeed=Number(opt.stableNeed||(fast?2:3));
    const stepRatio=Number(opt.stepRatio||(fast?1.35:.75));
    const minStep=Number(opt.minStep||(fast?900:500));
    const blockMap={};
    const scrollTarget=getMainScrollTarget();
    ...
    for(let i=0;i<maxLoops;i++){
        ...
        const nextTop=Math.min(maxTop,currentTop+Math.max(minStep,window.innerHeight*stepRatio));
        ...
        if(stableTimes>=stableNeed)break;
    }
    ...
    if(restoreWaitMs>0)await wait(restoreWaitMs);
...
function getCurrentPageSizeSetting(){
    const text=norm(document.body&&document.body.innerText?document.body.innerText:"");
    const matches=[...text.matchAll(/\b(20|50|100)\s*\/\s*Page\b/gi)]
        .map(m=>Number(m[1]||0))
        .filter(n=>n>0);
    return matches.length?matches[matches.length-1]:0;
}

function getCurrentPaginationExpectedCount(){
    const text=norm(document.body&&document.body.innerText?document.body.innerText:"").replace(/\s+/g," ");
    const m=text.match(/\b(\d{1,5})\s*[-–]\s*(\d{1,5})\s+(?:of|\/)\s+(\d{1,7})\b/i);
    if(!m)return 0;
    ...
    if(start>0&&end>=start&&total>=end)return end-start+1;
    return 0;
}

function setScrollTargetTopValue(target,top){
    const isWindowScroll=
...
function getScrollTargetTopValue(target){
    const isWindowScroll=
        target===document.scrollingElement ||
        target===document.documentElement ||
        target===document.body;

    return isWindowScroll?window.scrollY:target.scrollTop;
}

async function hasEnabledNextPageForCompletenessCheck(){
    const scrollTarget=getMainScrollTarget();
    const originalTop=getScrollTargetTopValue(scrollTarget);

    await scrollMainToBottom({fast:true,waitMs:140});

    const hasNext=findDynamicNextCandidates(5).some(btn=>btn&&!isDisabledLike(btn));

    setScrollTargetTopValue(scrollTarget,originalTop);
    await wait(80);

    return hasNext;
}

async function getOrderCollectionCompleteness(collectedCount){
    const rangeExpected=getCurrentPaginationExpectedCount();
    const pageSize=getCurrentPageSizeSetting();
    let hasNext=false;

    if(!rangeExpected&&pageSize>=50){
        hasNext=await hasEnabledNextPageForCompletenessCheck();
    }

    const expected=rangeExpected||((pageSize>=50&&hasNext)?pageSize:0);
    const suspicious=expected>=50&&collectedCount<expected;

    return {
        expected,
        pageSize,
        hasNext,
        missing:expected?Math.max(0,expected-collectedCount):0,
        suspicious
    };
}

function makeOrderCollectionWarning(label,completeness,collectedCount,passCount){
    if(!completeness||!completeness.suspicious)return "";

    return String(label||"当前页")+
        "可能漏扫：页面应有 "+completeness.expected+
        " 单，实际只采集到 "+collectedCount+
        " 单，少 "+completeness.missing+
        " 单。已自动补扫 "+passCount+
        " 次后仍不足，已停止本次操作。请刷新页面后重试，不能把这次结果当完整结果。";
}

async function collectOrderInfosForCriticalPage(label){
    const passes=[
        {topWaitMs:520,stepWaitMs:360,finalWaitMs:180,restoreWaitMs:0,maxLoops:30,stableNeed:3,stepRatio:.58,minStep:360},
        {topWaitMs:700,stepWaitMs:520,finalWaitMs:260,restoreWaitMs:0,maxLoops:46,stableNeed:4,stepRatio:.38,minStep:240},
        {topWaitMs:900,stepWaitMs:650,finalWaitMs:360,restoreWaitMs:0,maxLoops:64,stableNeed:5,stepRatio:.25,minStep:160}
    ];
    const infoMap={};
    ...
    for(let i=0;i<passes.length;i++){
        passCount=i+1;
        const blocks=await collectCurrentListOrderBlocksByScroll(passes[i]);
        blocks.map(infoFromBlock).filter(x=>x&&x.order).forEach(info=>{
            infoMap[info.order]=info;
        });

        const merged=Object.values(infoMap);
        completeness=await getOrderCollectionCompleteness(merged.length);

        if(!completeness.suspicious)break;

        updateStatus(String(label||"当前页")+"采集数量可疑：应有 "+completeness.expected+" 单，已采 "+merged.length+" 单，正在第 "+(i+2)+" 次补扫...");
        updateReviewStatus(String(label||"当前页")+"采集数量可疑：应有 "+completeness.expected+" 单，已采 "+merged.length+" 单，正在第 "+(i+2)+" 次补扫...");
    }

    const infos=Object.values(infoMap);
    const warning=makeOrderCollectionWarning(label,completeness,infos.length,passCount);
...
function extractOrderType(block){
    ...
    const typeText=lines(block.text).slice(0,28).join(" ");

    if(/\bfree\s+sample\b/i.test(typeText))return "Free Sample";
...
function isSampleOrderType(type){
    return /\bfree\s+sample\b/i.test(String(type||""));
}

function isReviewSampleOrder(info){
    if(!info)return false;
    if(isSampleOrderType(info.orderType))return true;
    const raw=String(info.rawText||"");
    return /\bfree\s+sample\b/i.test(raw)&&!/\bLIVE\b/i.test(String(info.orderType||""));
}

function displayOrderType(info){
    if(isReviewSampleOrder(info))return "样品订单";
    const type=String((info&&info.orderType)||"").trim();
    return type||"橱窗订单";
}

function reviewOrderCategoryLabel(info){
    if(isReviewLiveOrder(info))return "LIVE";
    if(isReviewSampleOrder(info))return "样品订单";
    return "橱窗订单";
}
...
function isUnsafePrintableTitle(title){
    const s=norm(title);
    const lower=s.toLowerCase();

    if(!s)return true;
    if(s.length<4)return true;
    if(/^未识别|^unrecognized/i.test(s))return true;
    if(/^577\d{15}$/.test(s))return true;
    if(isBadTitleLine(s))return true;
    if(lower==="item on screen"||lower==="product on screen")return true;
    if(/^seller note\s*:/i.test(s))return true;
    if(/^note\s*:/i.test(s))return true;

    return false;
}

function areComparableTitlesDifferent(a,b){
    const ak=normalizeTitleForCompare(a);
    const bk=normalizeTitleForCompare(b);

    if(!ak||!bk)return false;
    if(ak===bk)return false;
    if(ak.includes(bk)||bk.includes(ak))return false;

    return true;
}

function getPrintableTitleSafetyError(info,opt){
    opt=opt||{};

    if(opt.skipTitleSafety)return "";
    if(!info||!info.order)return "安全拦截：订单信息为空，已停止打印。";

    const title=norm(info.title);
    const shortId=info.shortId||String(info.order||"").slice(-6);

    if(isUnsafePrintableTitle(title)){
        return "安全拦截：订单 "+shortId+" 的 Title 为空、未识别或疑似抓错字段，已停止打印。当前 Title："+(title||"空");
    }

    const agg=getHistoryAggregate(info.order);
    const oldTitle=norm(agg&&agg.title);

    if(oldTitle&&areComparableTitlesDifferent(oldTitle,title)){
        return "安全拦截：订单 "+shortId+" 历史 Title 与当前 Title 不一致，已停止打印。历史："+oldTitle+"；当前："+title;
    }

    return "";
}

function preparePrintBatchSafety(infos,opt){
    opt=opt||{};

    const list=(infos||[]).filter(Boolean);

    if(opt.skipBatchSafety)return {ok:true,infos:list};
    ...
    list.forEach(info=>{
        const order=String(info&&info.order||"");

        if(!order)return;
        if(seen[order]){
            duplicates.push({
                order,
                shortId:info.shortId||order.slice(-6),
                firstTitle:seen[order].title||"",
                title:info.title||""
            });
            return;
        }
    ...
    if(duplicates.length){
        const preview=duplicates.slice(0,8).map(x=>
            (x.shortId||x.order.slice(-6))+"：同批重复订单号；首次 Title="+(x.firstTitle||"空")+"；重复 Title="+(x.title||"空")
        ).join("\n");

        return {
            ok:false,
            infos:unique,
            msg:"安全拦截：同一批打印中出现重复订单号，已停止整批打印。\n"+preview
        };
    }

    return {ok:true,infos:unique};
}

async function getRestPrintCountsMap(orderIds){
    ...
    const data=await firestoreRestFetch(":batchGet",{
        method:"POST",
        body:JSON.stringify({
            documents:ids.map(id=>firestoreRestDocName("tiktok_orders",id))
        })
    });

    (Array.isArray(data)?data:[]).forEach(row=>{
        if(!row||!row.found||!row.found.name)return;
        const id=decodeURIComponent(String(row.found.name).split("/").pop()||"");
        const doc=firestoreRestDocToPlain(row.found);
        out[id]={
            printCount:Number(doc.printCount||0)||0,
            title:doc.title||"",
            note:doc.note||doc.latestNote||"",
            orderStatus:doc.orderStatus||doc.latestStatus||"",
            latestPrintedAt:doc.latestPrintedAt||doc.updatedAt||"",
            latestReason:doc.latestReason||""
        };
    });
...
async function runCloudPrintPreflight(infos,opt){
    opt=opt||{};

    if(opt.skipCloudPreflight)return {ok:true};

    const list=(infos||[]).filter(x=>x&&x.order);
    const orderIds=[...new Set(list.map(x=>String(x.order)))];
    let cloudCounts={};

    try{
        cloudCounts=await getRestPrintCountsMap(orderIds);
    }catch(e){
        return {
            ok:false,
            msg:"安全拦截：云端打印次数预检查失败，批量打印已停止，避免重复出标签。\n错误："+(e&&e.message?e.message:String(e))
        };
    }

    const titleConflicts=list
        .map(info=>{
            const cloud=cloudCounts[String(info.order)]||{};
            const cloudTitle=norm(cloud.title);
            const currentTitle=norm(info.title);
            return Object.assign({},info,{_cloudTitle:cloudTitle,_currentTitle:currentTitle});
        })
        .filter(info=>info._cloudTitle&&areComparableTitlesDifferent(info._cloudTitle,info._currentTitle));

    if(titleConflicts.length){
        const preview=titleConflicts.slice(0,12).map(info=>
            (info.shortId||String(info.order).slice(-6))+
            " ｜ 云端 Title="+(info._cloudTitle||"空")+
            " ｜ 当前 Title="+(info._currentTitle||"空")
        ).join("\n");

        return {
            ok:false,
            msg:"安全拦截：以下订单云端 Title 与当前页面 Title 不一致，批量打印已停止。\n"+preview
        };
    }

    const printed=list
        .map(info=>{
            const local=getTotalPrintCount(info.order)||0;
            const cloudDoc=cloudCounts[String(info.order)]||{};
            const cloud=Number(cloudDoc.printCount||0);
            const count=Math.max(local,cloud);
            return Object.assign({},info,{
                _preflightPrintCount:count,
                _preflightLocalCount:local,
                _preflightCloudCount:cloud,
                _preflightLatestPrintedAt:cloudDoc.latestPrintedAt||"",
                _preflightLatestReason:cloudDoc.latestReason||""
            });
        })
        .filter(info=>info._preflightPrintCount>0);

    if(!printed.length)return {ok:true};

    const preview=printed.slice(0,12).map(info=>
        (info.shortId||String(info.order).slice(-6))+
        " ｜ 已打印 "+info._preflightPrintCount+" 次"+
        (info._preflightLatestPrintedAt?" ｜ 最近 "+info._preflightLatestPrintedAt:"")+
        " ｜ "+(info.title||"")
    ).join("\n");

    return {
        ok:false,
        msg:"安全拦截：以下订单本地或云端已有打印记录，批量打印已停止。\n"+
            preview+
            (printed.length>12?"\n...另有 "+(printed.length-12)+" 单":"")+
            "\n如确实需要重打，不能混在批量任务里，必须走单独补打流程。"
    };
}

async function printLabelsBatchWithSafety(infos,reason,opt){
    opt=opt||{};

    const batch=preparePrintBatchSafety(infos,opt);

    if(!batch.ok){
        updateStatus(batch.msg);
        updateReviewStatus(batch.msg);
        alert(batch.msg);
        return false;
    }

    const cloud=await runCloudPrintPreflight(batch.infos,opt);

    if(!cloud.ok){
        updateStatus(cloud.msg||"已取消打印。");
        updateReviewStatus(cloud.msg||"已取消打印。");
        return false;
    }

    return printLabelsBatch(batch.infos,reason,Object.assign({},opt,{skipBatchSafety:true}));
}
...
function validatePrintableInfo(info,opt){
    ...
    const titleSafetyError=getPrintableTitleSafetyError(info,opt);

    if(titleSafetyError){
        return {ok:false,msg:titleSafetyError};
    }
...
<div class="${noteClass}" style="top:21.2mm;height:7.6mm;">
...
function openPrintFrameWithLabels(labelsHtml){
    ...
    frame.style.border="2px solid #111";
    frame.style.borderRadius="8px";
    frame.style.background="#fff";
    frame.style.right="18px";
    frame.style.top="86px";
    frame.style.zIndex="2147483647";
    frame.style.boxShadow="0 8px 32px rgba(0,0,0,.35)";
...
body{background:#f5f5f5;font-family:Arial,"Microsoft YaHei",sans-serif;}
.printToolbar{position:sticky;top:0;z-index:999999;background:#111;color:#fff;padding:8px 10px;display:flex;align-items:center;justify-content:space-between;gap:8px;font:13px/1.35 Arial,"Microsoft YaHei",sans-serif;}
.printToolbar button{border:0;border-radius:5px;padding:6px 10px;cursor:pointer;font-weight:900;}
.printToolbar .send{background:#14b85a;color:#fff;}
.printToolbar .close{background:#fff;color:#111;}
.sheet{display:flex;flex-wrap:wrap;gap:8px;padding:10px;background:#f5f5f5;}
...
.qr{position:absolute;left:1.8mm;top:3mm;width:19mm;height:19mm;display:block;background:#fff;image-rendering:pixelated;image-rendering:crisp-edges;}
.row{position:absolute;overflow:hidden;text-align:left;font-weight:bold;line-height:1.08;}
.id{left:21.4mm;right:1.2mm;top:4.2mm;height:4.2mm;font-size:5.2pt;white-space:nowrap;overflow:hidden;}
.fullid{font-size:5.2pt;font-weight:bold;}
.last6inline{display:inline-block;font-size:11.6pt;font-weight:900;transform:scaleX(.94);transform-origin:left center;}
.title{left:21.8mm;right:1.2mm;top:9.5mm;height:6.4mm;font-size:7.4pt;line-height:.9;}
.time{left:21.8mm;right:1.2mm;top:17.2mm;height:3.4mm;font-size:7.8pt;white-space:nowrap;}
.note{position:absolute;overflow:hidden;text-align:left;left:2.8mm;right:1.2mm;display:flex;align-items:flex-end;justify-content:space-between;gap:1.5mm;font-size:10.2pt;font-weight:900;line-height:1.03;word-break:break-word;font-family:"Arial Black","Consolas","Courier New",Arial,sans-serif;letter-spacing:.18mm;}
.noteMain{flex:1 1 auto;overflow:hidden;word-break:break-word;}
.noteDigitsInline{flex:0 0 auto;font-size:14pt;line-height:.95;font-weight:900;font-family:"Consolas","Courier New",monospace;letter-spacing:.45mm;white-space:nowrap;padding-left:1mm;}
.numericNote{top:21.1mm !important;height:7.9mm !important;font-size:14pt;line-height:1.05;letter-spacing:.55mm;font-family:"Consolas","Courier New",monospace;}
.numericNote .noteMain{font-size:14pt;line-height:1.05;letter-spacing:.55mm;font-family:"Consolas","Courier New",monospace;}
@media screen{
  .label{background:white;border:1px dashed #aaa;box-shadow:0 1px 4px rgba(0,0,0,.12);page-break-after:auto;break-after:auto;}
}
@media print{
  .printToolbar{display:none!important;}
  body{background:white;}
  .sheet{display:block;padding:0;background:white;}
...
<div class="printToolbar">
  <div><b>TikTok 标签打印</b> ｜ 共 <span id="labelCount"></span> 张</div>
  <div>
    <button class="send" id="sendPrintBtn">发送打印任务</button>
    <button class="close" id="closePrintBtn">关闭</button>
  </div>
</div>
<div class="sheet">
${labelsHtml}
...
`);
    ...
    const labelCount=doc.getElementById("labelCount");
    const sendBtn=doc.getElementById("sendPrintBtn");
    const closeBtn=doc.getElementById("closePrintBtn");

    if(labelCount)labelCount.textContent=String(doc.querySelectorAll(".label").length);

    function waitForFrameImagesThen(fn){
    ...
    if(sendBtn){
        sendBtn.addEventListener("click",function(e){
            e.preventDefault();
            e.stopPropagation();
            waitForFrameImagesThen(function(){
                try{frameWin.focus();}catch(focusError){}
                setTimeout(function(){
                    try{
                        frameWin.print();
                        updateStatus("已触发浏览器打印窗口，请确认打印机任务。");
                    }catch(printError){
                        updateStatus("打印窗口触发失败："+(printError&&printError.message?printError.message:String(printError)));
                    }
                },120);
            });
        });
    ...
    try{
        frameWin.onafterprint=function(){
            setTimeout(closePrintFrame,700);
        };
    }catch(e){}
}
...
async function printSingleReprintWithConfirmation(info,reason,opt){
    opt=opt||{};
    ...
    const shortId=info.shortId||String(info.order).slice(-6);
    const localCount=getTotalPrintCount(info.order)||0;
    let cloudCount=null;
    let cloudTitle="";
    let cloudPrintedAt="";
    let cloudError="";
    ...
    try{
        const counts=await getRestPrintCountsMap([info.order]);
        const cloudDoc=counts[String(info.order)]||{};
        cloudCount=Number(cloudDoc.printCount||0)||0;
        cloudTitle=norm(cloudDoc.title);
        cloudPrintedAt=cloudDoc.latestPrintedAt||"";
    }catch(e){
    ...
    if(cloudTitle&&currentTitle&&areComparableTitlesDifferent(cloudTitle,currentTitle)){
        const msg=
            "安全拦截：单独补打的当前 Title 与云端 Title 不一致，已停止。\n"+
            "订单："+shortId+"\n"+
            "云端 Title："+cloudTitle+"\n"+
            "当前 Title："+currentTitle;
        alert(msg);
        updateStatus(msg);
        return false;
    }

    const total=cloudCount===null?localCount:Math.max(localCount,cloudCount);
    const msg=
        "单独补打确认：这次只会打印 1 张标签。\n\n"+
        "订单号："+info.order+"\n"+
        "后六位："+shortId+"\n"+
        "Title："+(currentTitle||"空")+"\n"+
        "Note："+(effectiveNote(info)||"空")+"\n"+
        "本地打印次数："+localCount+" 次\n"+
        "云端打印次数："+(cloudCount===null?"读取失败":cloudCount+" 次")+
        (cloudPrintedAt?"\n最近云端打印："+cloudPrintedAt:"")+
        (cloudError?"\n云端读取错误："+cloudError:"")+
        "\n\n确认要单独补打这一张？";

    if(!confirm(msg)){
        updateStatus("已取消单独补打："+shortId);
        return false;
    }

    const ok=printLabel(info,reason||"单独补打",Object.assign({},opt,{force:true}));

    if(ok){
        updateStatus("已单独补打 1 张："+shortId+"；补打前已打印次数："+total+" 次。");
    }
    ...
    if(!p){
        ...
        p.innerHTML=
            ...
            '<div id="panelBody" style="padding:8px 10px;">'+
                ...
                '<button id="quickTodayAuctionBtn" style="width:100%;margin-bottom:6px;padding:8px;border:1px solid #0a7a0a;border-radius:6px;cursor:pointer;background:#d8f5d1;color:#064d06;font-weight:bold;">'+esc(tr("quickTodayAuction"))+'</button>'+
        ...
        document.getElementById("quickTodayAuctionBtn").onclick=function(e){
            e.preventDefault();e.stopPropagation();
            if(!ensureOperatorName())return;
            if(!requireListeningPausedForAction("快捷打印今日拍卖"))return;
            quickPrintTodayAuctionAndGiveaway();
        };
...
function normalizeMainPanelUiText(){
    ...
    const quickTodayBtn=document.getElementById("quickTodayAuctionBtn");
    if(quickTodayBtn)quickTodayBtn.textContent=tr("quickTodayAuction");
...
async function clickNextPageSmart(opt){
    opt=opt||{};
    const fast=!!opt.fast;
    const preWaitMs=Number(opt.preWaitMs||(fast?40:120));
    const bottomWaitMs=Number(opt.bottomWaitMs||(fast?220:800));
    const afterBottomWaitMs=Number(opt.afterBottomWaitMs||(fast?80:250));
    const missingCandidateWaitMs=Number(opt.missingCandidateWaitMs||(fast?220:600));
    const changeTimeoutMs=Number(opt.changeTimeoutMs||(fast?8000:16000));
    const afterChangeWaitMs=Number(opt.afterChangeWaitMs||(fast?250:2000));
    const retryWaitMs=Number(opt.retryWaitMs||(fast?150:500));
    ...
    try{
        ...
        await wait(preWaitMs);
        await scrollMainToBottom({fast,waitMs:bottomWaitMs});
        await wait(afterBottomWaitMs);
        ...
        if(!candidates.length){
            await wait(missingCandidateWaitMs);
        ...
        for(let i=0;i<candidates.length;i++){
            ...
            const changed=await waitReviewPageTurnChanged(oldTurnSig,changeTimeoutMs);
            await wait(afterChangeWaitMs);
...
function summarizeInfos(infos){
    ...
    const live=infos.filter(x=>isReviewLiveOrder(x)).length;
...
function updateListenButton(){
    ...
    const isLive=isReviewLiveOrder(o);
    const isSample=isReviewSampleOrder(o);
    ...
    if(liveFilter==="notLive"&&(isLive||isSample))return false;
    if(liveFilter==="sample"&&!isSample)return false;
...
function getSummaryFromInfos(infos){
    ...
    const liveOrders=allOrders.filter(x=>isReviewLiveOrder(x));
...
function ensurePrintFloatingWindow(){
    ...
    document.getElementById("tkConfirmPrintBtn").onclick=async function(){
        ...
        const ok=await printLabelsBatchWithSafety(sortCurrentPagePrintQueueByTitle(infos),"手动批量打印当前页面订单",{force:true});
        if(ok)renderPrintFloatingRows();
    };
        ...
        rowsBox.innerHTML=arr.map(o=>{
            ...
            return `
                <label style="display:block;border:2px solid ${borderColor};border-radius:8px;margin:8px 0;padding:10px;cursor:${disabled?"not-allowed":"pointer"};opacity:${disabled?".72":"1"};background:${bgColor};">
                    ...
                    <span style="margin-left:6px;color:#555;">${esc(displayOrderType(o))}</span>
...
function isQuickTodayAuctionPrintCandidate(info,targetDate){
    if(!info||!info.order)return false;
    if(info.isCanceled||info.orderStatus==="Canceled")return false;
    if(getTotalPrintCount(info.order)!==0)return false;
    if((info.orderDate||"")!==targetDate)return false;
    if(info.orderType!=="LIVE Auction"&&info.orderType!=="LIVE Giveaway")return false;
    if(!norm(effectiveNote(info)))return false;
    return true;
}

function getQuickTodayPageStopInfo(infos,targetDate){
    const dated=(infos||[]).filter(x=>x&&x.orderDate);
    const hasToday=dated.some(x=>x.orderDate===targetDate);
    const hasOlder=dated.some(x=>x.orderDate<targetDate);
    const hasNewer=dated.some(x=>x.orderDate>targetDate);

    if(!(infos||[]).length)return {stop:true,reason:"当前页没有订单"};
    if(hasOlder)return {stop:true,reason:"当前页已出现早于今天的订单"};
    if(dated.length&&!hasToday&&!hasNewer)return {stop:true,reason:"当前页不是今天订单"};
    return {stop:false,reason:""};
}

function showQuickTodayAuctionPrintConfirm(printable,stopReason,pageSummaries){
    const old=document.getElementById("tk-quick-today-print-confirm");
    if(old)old.remove();

    const list=(printable||[]).slice(0,18).map(info=>{
        return '<div style="border-bottom:1px solid #eee;padding:6px 0;">'+
            '<b>'+esc(info.shortId||String(info.order||"").slice(-6))+'</b>'+
            '<span style="margin-left:8px;color:#555;">'+esc(info.orderType||"")+'</span>'+
            '<span style="margin-left:8px;">'+esc(info.title||"")+'</span>'+
            '<div style="font-size:12px;color:#555;">Note: '+esc(effectiveNote(info)||"")+'</div>'+
        '</div>';
    }).join("");

    const summary=(pageSummaries||[]).map(x=>
        "第"+x.page+"页 总"+x.total+
        " / Auction "+x.auction+
        " / Giveaway "+x.giveaway+
        " / 命中 "+x.hit+
        (x.expected?(" / 应采 "+x.expected):"")+
        " / 采集 "+(x.passCount||1)+"轮"
    ).join("；");

    const win=document.createElement("div");
    win.id="tk-quick-today-print-confirm";
    win.style.cssText="position:fixed;left:50%;top:80px;transform:translateX(-50%);width:620px;max-width:92vw;max-height:82vh;z-index:2147483647;background:#fff;border:2px solid #111;border-radius:10px;box-shadow:0 8px 32px rgba(0,0,0,.35);font-family:Arial,'Microsoft YaHei',sans-serif;display:flex;flex-direction:column;overflow:hidden;";
    win.innerHTML=
        '<div id="tkQuickPrintHeader" style="background:#111;color:#fff;padding:10px 12px;font-weight:900;display:flex;justify-content:space-between;align-items:center;cursor:move;user-select:none;">'+
            '<span>快捷打印今日拍卖 v'+SCRIPT_VERSION+'</span>'+
            '<button id="tkQuickPrintClose" style="background:#fff;color:#111;border:0;border-radius:4px;padding:4px 10px;cursor:pointer;font-weight:bold;">关闭</button>'+
        '</div>'+
        '<div style="padding:12px;background:#fff7d6;border-bottom:1px solid #e6d28a;line-height:1.5;">'+
            '<div><b>扫描完成，待打印 '+esc((printable||[]).length)+' 单。</b></div>'+
            '<div>停止原因：'+esc(stopReason||"扫描结束")+'</div>'+
            '<div style="font-size:12px;color:#555;margin-top:4px;word-break:break-all;">'+esc(summary)+'</div>'+
        '</div>'+
        '<div style="padding:10px 12px;overflow:auto;flex:1;">'+
            (list||'<div style="color:#777;">没有订单明细</div>')+
            ((printable||[]).length>18?'<div style="padding:8px 0;color:#666;">还有 '+esc((printable||[]).length-18)+' 单未在此预览显示。</div>':"")+
        '</div>'+
        '<div style="padding:12px;border-top:1px solid #ddd;display:flex;gap:10px;justify-content:flex-end;background:#fff;">'+
            '<button id="tkQuickPrintCancel" style="padding:9px 14px;border:1px solid #888;border-radius:6px;background:#f5f5f5;cursor:pointer;font-weight:bold;">取消</button>'+
            '<button id="tkQuickPrintSend" style="padding:9px 18px;border:0;border-radius:6px;background:#111;color:#fff;cursor:pointer;font-weight:900;">发送打印任务（'+esc((printable||[]).length)+'单）</button>'+
        '</div>';

    document.body.appendChild(win);
    makeFloatingWindowDraggable(win,"#tkQuickPrintHeader");
    focusFloatingWorkWindow(win);

    document.getElementById("tkQuickPrintClose").onclick=function(){win.remove();};
    document.getElementById("tkQuickPrintCancel").onclick=function(){win.remove();};
    document.getElementById("tkQuickPrintSend").onclick=async function(e){
        e.preventDefault();
        e.stopPropagation();
        const ok=await printLabelsBatchWithSafety(printable,"快捷打印今日拍卖/Giveaway",{force:true});
        if(ok){
            updateStatus("快捷打印今日拍卖：已通过确认按钮发送打印任务 "+printable.length+" 单。");
            win.remove();
        }
    };
}

async function quickPrintTodayAuctionAndGiveaway(){
    if(quickTodayAuctionRunning){
        updateStatus("快捷打印今日拍卖正在运行，请等待当前任务结束。");
        return;
    }
    if(guardTikTokVerification("快捷打印今日拍卖已暂停"))return;
    if(!requireListeningPausedForAction("快捷打印今日拍卖"))return;

    quickTodayAuctionRunning=true;
    ...
    let page=1;
    let stopReason="";
    let stoppedForIncompleteCollection=false;
    const startedAt=Date.now();

    try{
        updateStatus("快捷打印今日拍卖：开始扫描 "+targetDate+" 的 LIVE Auction / LIVE Giveaway 订单...");

        while(page<=80){
            const pageStartedAt=Date.now();
            const collectResult=await collectOrderInfosForCriticalPage("快捷打印第 "+page+" 页");
            const pageInfos=collectResult.infos;
            if(collectResult.warning){
                stoppedForIncompleteCollection=true;
                stopReason=collectResult.warning;
                updateStatus(stopReason);
                alert(stopReason);
                break;
            }
            const pageCandidates=pageInfos.filter(info=>isQuickTodayAuctionPrintCandidate(info,targetDate));
            const pageMs=Date.now()-pageStartedAt;

            pageCandidates.forEach(info=>{
                found[info.order]=info;
            });

            const pageAuction=pageInfos.filter(x=>x.orderType==="LIVE Auction").length;
            const pageGiveaway=pageInfos.filter(x=>x.orderType==="LIVE Giveaway").length;
            const pageCanceled=pageInfos.filter(x=>x.isCanceled||x.orderStatus==="Canceled").length;
            const pagePrinted=pageInfos.filter(x=>getTotalPrintCount(x.order)>0).length;
            const pageNoNote=pageInfos.filter(x=>(x.orderType==="LIVE Auction"||x.orderType==="LIVE Giveaway")&&!norm(effectiveNote(x))).length;

            pageSummaries.push({
                page,
                total:pageInfos.length,
                auction:pageAuction,
                giveaway:pageGiveaway,
                hit:pageCandidates.length,
                canceled:pageCanceled,
                printed:pagePrinted,
                noNote:pageNoNote,
                expected:collectResult.completeness&&collectResult.completeness.expected?collectResult.completeness.expected:0,
                passCount:collectResult.passCount||1,
                ms:pageMs
            });

            updateStatus(
                "快捷打印今日拍卖：第 "+page+" 页扫描 "+pageInfos.length+
                " 单，Auction "+pageAuction+
                "，Giveaway "+pageGiveaway+
                "，本页命中 "+pageCandidates.length+
                "，累计待打印 "+Object.keys(found).length+
                (collectResult.completeness&&collectResult.completeness.expected?("，应采 "+collectResult.completeness.expected):"")+
                "，采集 "+(collectResult.passCount||1)+" 轮"+
                "，本页耗时 "+(pageMs/1000).toFixed(1)+" 秒"+
                "。"
            );

            const stop=getQuickTodayPageStopInfo(pageInfos,targetDate);
            if(stop.stop){
                stopReason=stop.reason;
                break;
            }

            const turn=await clickNextPageSmart({
                fast:true,
                preWaitMs:35,
                bottomWaitMs:220,
                afterBottomWaitMs:70,
                missingCandidateWaitMs:220,
                changeTimeoutMs:8000,
                afterChangeWaitMs:220,
                retryWaitMs:120
            });
            if(!turn||!turn.ok){
                stopReason=turn&&turn.reason?turn.reason:"没有更多下一页";
                break;
            }
        ...
        if(page>80)stopReason="已达到最多 80 页保护上限";

        if(stoppedForIncompleteCollection){
            updateStatus("快捷打印今日拍卖已停止：检测到页面采集不完整，未生成打印任务。");
            return;
        }

        const printable=sortCurrentPagePrintQueueByTitle(Object.values(found));
        const elapsedText=((Date.now()-startedAt)/1000).toFixed(1)+" 秒";
        const summaryText=pageSummaries.map(x=>
            "第"+x.page+"页: 总"+x.total+
            " / Auction "+x.auction+
            " / Giveaway "+x.giveaway+
            " / 命中 "+x.hit+
            ...
            (x.expected?(" / 应采 "+x.expected):"")+
            " / 采集 "+(x.passCount||1)+"轮"+
            " / "+((x.ms||0)/1000).toFixed(1)+"秒"
        ).join("；");

        if(!printable.length){
            updateStatus("快捷打印今日拍卖完成：耗时 "+elapsedText+"，没有符合条件的订单。停止原因："+(stopReason||"扫描结束")+"。"+summaryText);
            alert("快捷打印今日拍卖：没有符合条件的订单。\n条件：今天 + LIVE Auction/Giveaway + 非Canceled + 打印次数0 + 有Note。\n停止原因："+(stopReason||"扫描结束"));
            return;
        }

        updateStatus("快捷打印今日拍卖：扫描完成，耗时 "+elapsedText+"，已生成 "+printable.length+" 单待打印。请在弹窗里点击发送打印任务。停止原因："+(stopReason||"扫描结束")+"。");
        showQuickTodayAuctionPrintConfirm(printable,stopReason,pageSummaries);
    }catch(e){
        const msg="快捷打印今日拍卖异常："+(e&&e.message?e.message:String(e));
        updateStatus(msg);
        alert(msg);
    }finally{
        quickTodayAuctionRunning=false;
        updateStablePanelSoon();
    }
}
...
function renderCloudHistoryRows(){
    ...
    rowsBox.innerHTML=arr.map(o=>{
        ...
        return '<div style="border:1px solid '+(o.orderStatus==="Canceled"?'#ff3333':'#d0d0d0')+';background:'+(o.orderStatus==="Canceled"?'#fff1f1':'#fff')+';border-radius:8px;padding:10px;margin:8px 0;">'+
            ...
            '<div style="font-size:12px;color:#555;word-break:break-all;margin-top:4px;">订单号：'+esc(o.order)+' ｜ 类型：'+esc(displayOrderType(o))+' ｜ 状态：'+esc(o.orderStatus||'')+'</div>'+ 
    ...
    rowsBox.querySelectorAll('.tkHistReprint').forEach(btn=>{
        btn.onclick=async function(e){
            ...
            const ok=await printSingleReprintWithConfirmation(info,'历史云端单独补打',{force:true});
            if(ok)setTimeout(loadAndRenderCloudHistory,1200);
        };
    });
            ...
            try{
                ...
                box.innerHTML=warn+(logs.length?logs.map((x,i)=>{
                    ...
                    return '<div style="border-bottom:1px solid #d7e3ff;padding:6px 0;">'+
                        ...
                        '<div>状态：'+esc(x.orderStatus||x.latestStatus||"")+' ｜ 类型：'+esc(displayOrderType(x))+'</div>'+
            ...
            try{
                ...
                box.innerHTML=logs.length?logs.map(x=>{
                    return '<div style="border-bottom:1px solid #e5e5e5;padding:5px 0;">'+
                        ...
                        '<div>状态：'+esc(x.latestReviewStatus||x.orderStatus||"")+' ｜ 类型：'+esc(displayOrderType(x))+' ｜ 快递：'+esc(x.latestReviewTrackingText||x.trackingText||"无/未识别")+'</div>'+
...
function isReviewLiveOrder(info){
    ...
    return !!(info&&/\bLIVE\b/i.test(type));
}
...
function addReviewDateBucketInfo(bucket,info,status){
    ...
    const sample=isReviewSampleOrder(info);

    bucket.total++;
    bucket.totalGmv+=gmv;

    if(live)bucket.live++;
    else if(sample)bucket.sample++;
    else bucket.notLive++;
    ...
    if(canceled){
        ...
        bucket.nonCanceledSample++;
        bucket.nonCanceledSampleGmv+=gmv;
    }else{
...
function buildReviewResult(scanId,stopDate,reason){
    ...
    let nonCanceledSample=0;
    let nonCanceledSampleGmv=0;
...
function makeCompactReviewHistoryItem(result){
    ...
    return {
        ...
        nonCanceledSample:Number(result.nonCanceledSample||0),
        nonCanceledSampleGmv:Number(result.nonCanceledSampleGmv||0),
...
function renderReviewProductAnalysis(){
    ...
    const sampleOrders=all.reduce((s,x)=>s+x.sampleOrders,0);
    const sampleUnits=all.reduce((s,x)=>s+x.sampleUnits,0);
    const sampleGmv=all.reduce((s,x)=>s+x.sampleGmv,0);
    ...
    if(summary){
        ...
        summary.innerHTML=
            ...
            ' ｜ 样品订单 '+esc(sampleOrders)+' 单 / '+esc(sampleUnits)+' 个 / '+esc(formatMoneyAmount(sampleGmv))+
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px;background:white;">
            <thead>
                <tr>
                    ...
                    <th style="position:sticky;top:0;background:#f6f8fa;border:1px solid #d8dee4;padding:6px;">样品单/件/GMV</th>
            ...
            <tbody>
                ${rows.map((row,i)=>`
                    <tr class="tkReviewProductRow" data-index="${i}" style="cursor:pointer;background:${i%2?'#fff':'#fbfdff'};">
                        ...
                        <td style="border:1px solid #d8dee4;padding:7px;font-weight:900;">${esc(row.sampleOrders)} / ${esc(row.sampleUnits)} / ${esc(formatMoneyAmount(row.sampleGmv))}</td>
...
function renderReviewProductDetail(row){
    ...
    if(summary){
        summary.innerHTML=
            ...
            ' ｜ 样品订单 '+esc(row.sampleOrders)+' 单 / '+esc(row.sampleUnits)+' 个 / '+esc(formatMoneyAmount(row.sampleGmv))+
    ...
    box.innerHTML=`
        <table style="width:100%;border-collapse:collapse;font-size:12px;">
            ...
            <tbody>
                ${orders.map(info=>{
                    const status=info.orderStatus||"Unknown";
                    const type=reviewOrderCategoryLabel(info);
                    const note=effectiveNote(info)||"";
                    return `<tr>
...
function makeReviewProductAnalysisText(){
    ...
    const lines=[
        ...
        "生成时间："+now(),
        "Title\t总单\t总件数\t总GMV\tLIVE单\tLIVE件\tLIVE GMV\t橱窗单\t橱窗件\t橱窗GMV\t样品单\t样品件\t样品GMV\tCanceled单\tCanceled件\tCanceled GMV\t状态"
    ];
    ...
    rows.forEach(row=>{
        lines.push([
            ...
            row.sampleOrders,
            row.sampleUnits,
            formatMoneyAmount(row.sampleGmv),
            row.canceledOrders,
            row.canceledUnits,
...
function renderReviewPageStats(){
    ...
    if((!reviewScannedInfos||!reviewScannedInfos.length)&&reviewLastResult&&reviewLastResult.summaryOnly&&Array.isArray(reviewLastResult.daily)&&reviewLastResult.daily.length){
        const rows=reviewLastResult.daily.map(x=>`<tr>
            ...
            <td>${esc(x.nonCanceledSample||0)}</td>
            <td>${esc(formatMoneyAmount(x.nonCanceledSampleGmv||0))}</td>
        </tr>`).join("");
        el.innerHTML=`
            <style>
            ...
            <div style="overflow-x:auto;background:white;border:1px solid #d0d7de;border-radius:8px;padding:9px;">
                ...
                <table><thead><tr><th>日期</th><th>总单</th><th>总GMV</th><th>Canceled</th><th>Canceled GMV</th><th>非Canceled LIVE</th><th>LIVE GMV</th><th>非Canceled 橱窗订单</th><th>橱窗订单 GMV</th><th>非Canceled 样品订单</th><th>样品订单 GMV</th></tr></thead><tbody>${rows}</tbody></table>
            </div>
        `;
        return;
    }
    ...
    let live=0,sample=0,notLive=0,canceled=0,withNote=0,noNote=0,withTracking=0,noTracking=0;
    let printed=0,noteAndPrinted=0,noteButNoPrint=0,printedButNoteDiff=0;
    let totalGmv=0,canceledGmv=0,nonCanceledLive=0,nonCanceledLiveGmv=0,nonCanceledSample=0,nonCanceledSampleGmv=0,nonCanceledNotLive=0,nonCanceledNotLiveGmv=0;
    ...
    infos.forEach(info=>{
        ...
        const isSample=isReviewSampleOrder(info);
        const pCount=getTotalPrintCount(info.order)||0;
        const agg=getHistoryAggregate(info.order)||{};
        const hNote=stripPrintedMark(agg.note||agg.systemNote||"");
        if(!dateMap[date])dateMap[date]={date,total:0,totalGmv:0,live:0,sample:0,notLive:0,canceled:0,canceledGmv:0,nonCanceledLive:0,nonCanceledLiveGmv:0,nonCanceledSample:0,nonCanceledSampleGmv:0,nonCanceledNotLive:0,nonCanceledNotLiveGmv:0,byStatus:{},withNote:0,noNote:0,withTracking:0,noTracking:0,printed:0,notePrintedMatch:0,noteNoPrint:0,printedNoteDiff:0,abnormal:0};
        const d=dateMap[date];
        d.total++;
        d.totalGmv+=gmv;
        totalGmv+=gmv;
        addMap(d.byStatus,status); addMap(statusMap,status);
        if(isLive){live++;d.live++;}else if(isSample){sample++;d.sample++;}else{notLive++;d.notLive++;}
        if(isCanceled){
            canceled++;
            canceledGmv+=gmv;
            d.canceled++;
    ...
    const nonCanceledGmv=nonCanceledLiveGmv+nonCanceledSampleGmv+nonCanceledNotLiveGmv;
    const collectionWarnings=reviewPageStats.filter(x=>x&&x.collectWarning);
    ...
    textLines.push('订单类型统计：总LIVE '+live+'；总橱窗订单 '+notLive+'；总样品订单 '+sample+'；非Canceled LIVE '+nonCanceledLive+' 单 / '+formatMoneyAmount(nonCanceledLiveGmv)+'；非Canceled 橱窗订单 '+nonCanceledNotLive+' 单 / '+formatMoneyAmount(nonCanceledNotLiveGmv)+'；非Canceled 样品订单 '+nonCanceledSample+' 单 / '+formatMoneyAmount(nonCanceledSampleGmv));
    ...
    el.innerHTML=`
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(135px,1fr));gap:8px;margin-bottom:9px;">
            ...
            <div class="rv-card"><div class="rv-label">非Canceled GMV</div><div class="rv-value" style="font-size:20px;">${esc(formatMoneyAmount(nonCanceledGmv))}</div><div style="font-size:11px;color:#777;">LIVE/橱窗/样品合计</div></div>
        ...
        ${collectionWarnings.length?'<div style="background:#fff0f0;border:1px solid #ff4d4f;color:#a40000;border-radius:8px;padding:9px;margin-bottom:9px;font-weight:900;">采集风险：'+esc(collectionWarnings.map(x=>'第'+x.page+'页 '+x.collectWarning).join('；'))+'</div>':""}
        ...
        <div class="rv-section" style="overflow-x:auto;"><div style="font-weight:900;margin-bottom:7px;">按日期统计</div><table><thead><tr><th>日期</th><th>总单</th><th>总GMV</th><th>Canceled</th><th>Canceled GMV</th><th>非Canceled LIVE</th><th>LIVE GMV</th><th>非Canceled 橱窗订单</th><th>橱窗订单 GMV</th><th>非Canceled 样品订单</th><th>样品订单 GMV</th><th>状态</th><th>有Note</th><th>Note空</th><th>已打印</th><th>Note未打印</th><th>Note不一致</th><th>异常</th></tr></thead><tbody>${dateRows}</tbody></table></div>
...
async function startReviewRecentOrdersCoreV104(){
    ...
    let stoppedForIncompleteCollection=false;
    let incompleteCollectionReason="";
    ...
    for(let page=1;page<=REVIEW_MAX_PAGES;page++){
        ...
        const collectResult=await collectOrderInfosForCriticalPage("复核第 "+page+" 页");
        const infos=collectResult.infos;
        if(collectResult.warning){
            stoppedForIncompleteCollection=true;
            incompleteCollectionReason=collectResult.warning;
            reviewPageStats.push({
                ...
                unknown:0,
                expected:collectResult.completeness&&collectResult.completeness.expected?collectResult.completeness.expected:0,
                collectPasses:collectResult.passCount||1,
                collectWarning:collectResult.warning,
                diffCount:0
            });
            renderReviewPageStats();
            updateReviewStatus(collectResult.warning);
            alert(collectResult.warning);
            break;
        }
        ...
        updateReviewStatus(
            ...
            (collectResult.completeness&&collectResult.completeness.expected?(" 条，应采 "+collectResult.completeness.expected+" 单"):"")+
            "，采集 "+(collectResult.passCount||1)+" 轮"+
            "。累计范围内订单 "+reviewScannedInfos.length+
    ...
    reviewLastResult=buildReviewResult(scanId,stopDate,stoppedForIncompleteCollection?("扫描中止："+incompleteCollectionReason):"扫描完成");
    ...
    if(stoppedForIncompleteCollection){
        updateReviewStatus(
            "复核中止：检测到页面采集不完整，不能把这次结果当完整结果。原因："+incompleteCollectionReason+
            " 已扫范围内订单 "+finalTotal+
            " 个；差异 "+reviewDifferences.length+
            " 条。复核历史："+(finalHistorySave&&finalHistorySave.ok?("已保存排查记录（"+finalHistorySave.level+"，订单 "+finalHistorySave.orders+"，差异 "+finalHistorySave.diffs+"）"):"保存失败，已保留摘要")+
            "。请刷新页面后重新复核。"
        );
        return;
    }
            ...
            <label style="display:block;border:2px solid ${color};background:${bg};border-radius:9px;padding:10px 12px;margin:10px 0;cursor:pointer;">
                <div style="display:flex;gap:8px;align-items:center;justify-content:space-between;">
                    <div>
                        ...
                        <span style="margin-left:8px;color:#555;">${esc(displayOrderType(d.info||d))}</span>
...
async function reviewBatchPrint(){
    ...
    const ok=await printLabelsBatchWithSafety(infos,"复核后按当前页面Note打印",{force:true});
    if(ok){
        reviewDifferences=reviewDifferences.filter(d=>!selected.includes(d));
        renderReviewRows();
    }
    ...
    box.innerHTML=`
        ...
        ${alerts.map(a=>`
            <div style="border-top:1px dashed #ff9999;padding-top:5px;margin-top:5px;">
                ...
                <div>类型：${esc(displayOrderType(a))}</div>
