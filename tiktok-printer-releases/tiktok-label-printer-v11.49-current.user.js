// ==UserScript==
// @name         WINDOWS电脑 TikTok标签 v11.49 清理稳定版
// @name:zh-CN   WINDOWS电脑 TikTok标签 v11.49 清理稳定版
// @namespace    local-tiktok-label-printer-windows
// @version      11.49
// @description  v11.49：批量打印遇到已打印订单直接停止；单独补打保留，但必须确认订单号、Title、Note 和打印次数。
// @description:zh-CN v11.49：批量打印遇到已打印订单直接停止；单独补打保留，但必须确认订单号、Title、Note 和打印次数。
...
async function quickPrintTodayAuctionAndGiveaway(){
    ...
    try{
        ...
        if(!printable.length){
            updateStatus("快捷打印今日拍卖完成：没有符合条件的订单。停止原因："+(stopReason||"扫描结束")+"。"+summaryText);
            alert("快捷打印今日拍卖：没有符合条件的订单。\n条件：今天 + LIVE Auction/Giveaway + 非Canceled + 打印次数0 + 有Note。\n停止原因："+(stopReason||"扫描结束"));
            return;
        }

        updateStatus("快捷打印今日拍卖：扫描完成，已生成 "+printable.length+" 单待打印。请在弹窗里点击发送打印任务。停止原因："+(stopReason||"扫描结束")+"。");
