// ==UserScript==
// @name         WINDOWS电脑 TikTok标签 v11.47 清理稳定版
// @name:zh-CN   WINDOWS电脑 TikTok标签 v11.47 清理稳定版
// @namespace    local-tiktok-label-printer-windows
// @version      11.47
// @description  v11.47：收窄订单号后 6 位数字，增加右侧安全边距，避免最后一位打印出格
// @description:zh-CN v11.47：收窄订单号后 6 位数字，增加右侧安全边距，避免最后一位打印出格
...
function showQuickTodayAuctionPrintConfirm(printable,stopReason,pageSummaries){
    ...
    document.getElementById("tkQuickPrintSend").onclick=function(e){
        e.preventDefault();
        e.stopPropagation();
        const ok=printLabelsBatch(printable,"快捷打印今日拍卖/Giveaway",{force:true});
        if(ok){
