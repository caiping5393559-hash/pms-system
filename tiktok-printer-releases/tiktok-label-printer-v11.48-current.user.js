// ==UserScript==
// @name         WINDOWS电脑 TikTok标签 v11.48 清理稳定版
// @name:zh-CN   WINDOWS电脑 TikTok标签 v11.48 清理稳定版
// @namespace    local-tiktok-label-printer-windows
// @version      11.48
// @description  v11.48：新增打印前安全闸，拦截同批重复订单、Title 可疑/冲突，并对云端已打印订单二次确认
// @description:zh-CN v11.48：新增打印前安全闸，拦截同批重复订单、Title 可疑/冲突，并对云端已打印订单二次确认
...
async function getRestPrintCountsMap(orderIds){
    ...
    (Array.isArray(data)?data:[]).forEach(row=>{
        ...
        out[id]=firestoreRestNumber(row.found.fields&&row.found.fields.printCount);
    });
...
async function runCloudPrintPreflight(infos,opt){
    ...
    try{
        ...
        const msg="云端打印次数预检查失败，继续打印可能造成重复标签。\n错误："+(e&&e.message?e.message:String(e))+"\n\n点“确定”继续打印；点“取消”停止。";
        const ok=confirm(msg);

        if(!ok)return {ok:false,msg:"已取消打印：云端预检查失败。"};
        return {ok:true};
    }
    ...
    const printed=list
        .map(info=>{
            ...
            const cloud=Number(cloudCounts[String(info.order)]||0);
            const count=Math.max(local,cloud);
            return Object.assign({},info,{_preflightPrintCount:count,_preflightLocalCount:local,_preflightCloudCount:cloud});
        })
    ...
    const ok=confirm(
        "安全确认：以下订单本地或云端已有打印记录。\n\n"+
        preview+
        (printed.length>12?"\n...另有 "+(printed.length-12)+" 单":"")+
        "\n\n继续会再次打印这些订单。确定继续？"
    );

    if(!ok)return {ok:false,msg:"已取消打印：存在已打印订单。"};
