...
function setupPropertyRoomUi(){
  ...
  if(section){
    ...
    if(p) p.textContent = '先添加或选择房源，再进入该房源的房间设置。房间默认只填 Airbnb iCal；需要其他平台时再展开。';
  }
  ...
  style.textContent = `
    ...
    .property-room-layout{display:grid;grid-template-columns:270px minmax(0,1fr);gap:16px;align-items:start}
    .property-sidebar{border:1px solid var(--line);border-radius:8px;background:#fff;padding:12px;position:sticky;top:92px}
    .property-list{display:grid;gap:8px;margin-top:10px}
    .property-list-item{width:100%;border:1px solid var(--line);background:#fff;border-radius:8px;padding:10px;text-align:left;cursor:pointer}
    .property-list-item.active{border-color:var(--primary);background:#f0fdfa;box-shadow:inset 3px 0 0 var(--primary)}
    .property-list-title{display:block;font-weight:900;color:var(--text);line-height:1.25}
    .property-list-meta{display:block;color:var(--muted);font-size:12px;margin-top:5px}
    ...
    .room-compact-head{display:grid;grid-template-columns:minmax(160px,1fr) 110px 170px auto;gap:8px;align-items:end}
    ...
    @media(max-width:900px){.property-room-layout{grid-template-columns:1fr}.property-sidebar{position:static}.room-compact-head,.room-ical-primary{grid-template-columns:1fr}}
  `;
...
function renderRoomCard(room, prop){
  ...
  return `<div class="room-setting-card">
    <div class="room-compact-head">
      ...
      <div><label>所属房源</label><select onchange="moveRoomToProperty('${esc(room.id)}',this.value)">${propertyOptions(roomProperty)}</select></div>
